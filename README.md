...
5. 在主机名旁点击 **＋**，填写账号和密码；密码会在需要时使用配置加密主口令保存，也可以选择私钥认证。配置名会自动生成为 `主机名(账号)`：**主机名取别名**（如 `ws1(alice)`、`gateway(ops_carol)`），别名是中文或没配时退回 IP（如 `192.0.2.30(carol)`）。主机节点上的 ＋ 是**追加账号**，不会覆盖同 IP 下已有的账号。
6. 首次连接时核对并确认服务器主机密钥指纹。

主机节点（相同 IP 合并的那一项）上的 **🗑** 会删除整组账号配置（同一 IP 下的所有账号及其挂载），账号节点上的 **🗑** 只删该账号；两者删除前都会弹确认框，删除后会一并停掉对应的同步任务并清掉历史目录与本地副本配对；任一节点右键选择 **打开配置**，会打开 `~/.safs/config.json` 并把光标定位到这条记录（视图标题栏的 **打开配置** 按当前选中项定位）。
...
旧版本生成的配置名（`IP(账号)`、`账号_主机名`、更早的 `主机@账号`）会在启动时自动迁移成 `主机名(账号)`，挂载、历史目录、转发开关、同步任务与同步配对会一起改名。名字里的括号在 URI 里写成 `_`（`ws1(alice)` → `safs://ws1_alice/…`），完整配置名另存于 URI 的 `?mount=`，即使它被转义丢失也能从 authority 还原。改名后已打开的窗口会被更新成新名字（左下角远程指示器显示的就是它）：多根工作区直接替换文件夹，单文件夹窗口会问一句是否用新名字重新打开；旧名字的窗口/标签页即使不重开也仍然能打开。
...
上传或下载大文件（1 MiB 以上）中断后，再执行一次同一个文件的传输会**从断点继续**，不会从 0 重来。进度提示会显示已有字节数并标注「续传」。

- 续传不依赖 rsync，也不需要远端安装任何东西：SFTP 协议的读写报文自带字节偏移，SAFS 直接用它做定位读写；只有网关没有 SFTP 子系统、退回 `scp` 通道时才退化为整传。
- 中断时残片保留在目标旁边（上传是远端同目录的 `.<文件名>.safs-part-<哈希>`，下载是本地同目录的 `<文件名>.safs-part-<哈希>`），成功提交后会改名到最终位置。
- **源文件变过就不续传**：残片名里编进了源文件的 size+mtime，远端（或本地）文件在两次尝试之间被改动过，名字就对不上，自动整传——避免把两个版本的字节拼成一个看起来正常的坏文件。
- 小于 1 MiB 的残片不留（续传收益不抵一个垃圾文件），残片超过 7 天会在下一次传输时顺手清掉。
