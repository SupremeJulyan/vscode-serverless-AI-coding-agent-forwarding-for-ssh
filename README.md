...
旧版本生成的 `账号_主机名`（以及更早的 `主机@账号`）配置名会在启动时自动迁移成 `IP(账号)`，挂载、历史目录、转发开关、同步任务与同步配对会一起改名；已保存的 `safs://` 远程窗口/标签页 URI 里仍是旧名字，SAFS 会记住改名映射并继续解析，不需要重新打开。名字里的括号在 URI 里写成 `_`（`10.68.0.1(zhuyuan)` → `safs://10.68.0.1_zhuyuan/…`），完整配置名另存于 URI 的 `?mount=`，即使它被转义丢失也能从 authority 还原。
