...
- 配置名改为 `主机名(账号)`：主机名取 ASCII 别名（如 `ls(zhuyuan)`、`gsx01(nsgsx_zhangk)`），别名是中文或没配时退回 IP（如 `10.38.36.8(yewenlong)`）。旧名字（`IP(账号)`、`账号_主机名`、更早的 `主机@账号`）在启动时自动迁移，挂载、历史目录、转发开关、同步任务与同步配对一起改名。
- 改名后已打开的窗口会把 `safs://` 工作区 URI 换成当前名字：左下角远程指示器显示的就是它。多根工作区直接替换文件夹；单文件夹窗口 VS Code 不允许改文件夹列表，会问一句是否用新名字重新打开（会重新加载窗口）。括号在 authority 里写成 `_`（`ls(zhuyuan)` → `safs://ls_zhuyuan/…`）。
- 修复配置改名后远程目录打不开（资源管理器里带感叹号）：VS Code 会把 authority 里的括号百分号转义（`safs://10.44.9.4%28zhuyuan%29/…`），而解析时没有还原，于是找不到挂载。现在解析 URI 时先还原 authority 的转义，并且生成的 authority 直接把括号写成 `_`（`10.68.0.1(zhuyuan)` → `safs://10.68.0.1_zhuyuan/…`），不再让 `%28` 出现在远程指示器和已保存的窗口状态里。
- URI 里的 `?mount=` 只作参考：VS Code 会把 query 里的 `=` 转义成 `%3D`（`?mount%3Dhost@user`），取不到时改为从 authority 还原名字。启动时登记两类别名——历史命名规则下的旧配置名（`账号_别名`、`账号_IP`、`别名@账号`、`IP@账号`）和当前配置名的 authority 转义形式（`10.68.0.1_zhuyuan` → `10.68.0.1(zhuyuan)`）——已保存的窗口、标签页、最近打开都不需要重开就能解析到当前配置；只登记当前不存在的名字，不会遮蔽现有配置。
