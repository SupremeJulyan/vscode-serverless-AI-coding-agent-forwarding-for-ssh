...
- 配置名改为 `IP(账号)`（例如 `10.68.0.1(zhuyuan)`）：主机别名只作为主机节点的显示标签，不再参与命名；同一 IP 的多个账号一眼能分清，`config.json` 里按 IP 也能直接对上。旧名字（`账号_主机名`、更早的 `主机@账号`）在启动时自动迁移，挂载、历史目录、转发开关、同步任务与同步配对一起改名。
- `safs://` 的 authority 允许括号（RFC 3986 sub-delims，且能被 URI 解析器原样保留），所以新名字在 VS Code 远程指示器里仍然可读；含 `#` 的去重名等不安全字符照旧退回十六进制。注意：改名后已打开的远程窗口用的是旧名字，需要从 SAFS 面板重新打开一次。
- 主机节点（相同 IP 合并的那一项）新增删除按钮：一次删掉该主机下的全部账号及其挂载配置，有连接在用时会先确认断开；账号节点仍是只删自己那条配置。
