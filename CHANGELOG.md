...
- 可视化上传/下载支持大文件断点续传：中断后再次传输同一个文件，从上次的字节位置继续，不再从 0 重来。
  - 上传：残片改为确定性命名（远端同目录 `.<文件名>.safs-part-<哈希>`），SFTP 定位写从残片长度起步；下载：残片名同样确定性（本地同目录 `<文件名>.safs-part-<哈希>`），读取用 `createReadStream({ start })` 从断点开始。此前上传用随机名 `.safs-upload-<uuid>.part` 且写位置写死 0，下载每次都新建唯一临时名，所以都不可能续传。
  - 归属与版本校验：残片名由「会话令牌 + 源文件名 + 源 size+mtime」哈希而来。源文件在两次尝试之间被改过（远端或本地）名字就对不上，自动整传——避免把两个版本的字节拼成看起来正常的坏文件。残片自身的 size 只作为续传起点（残片天生没传完），另加一条「残片 mtime 与源相同则拒绝续传」兜住同大小同时间的内容改写。
  - 残片保留：失败/取消时不再一律删除——大于 1 MiB 的残片留下当续传起点（进度与日志都会说明），小残片直接删；超过 7 天的陈旧残片在下一次传输时顺手清理（含扩展重载后因为会话令牌不同而永远无法续传的旧残片）。
  - SCP 回退通道显式降级：`scp -f`/`scp -t` 没有字节偏移能力，`readFileStream(start)` 与 `writeFileStream({ startOffset })` 在这条通道上直接报错而不是静默全量，判定层也会返回「不支持续传」。
  - 进度显示随之修正：续传时通知从已有字节数起算并标注「续传中」，`bytes` 仍只统计本轮新传的字节。
- 同步引擎的单文件下载也走同一套续传（此前每次用 `pid+随机数` 临时名，中断即全量重来）。
- 顺带修正：`writeFile`/`writeFileStream` 的打开标志推导里 `overwrite=false, create=false` 分支原本也走 `r+`，现在等价写法合并，行为不变。

## 1.9.5

- 修复打开远程目录时的 `Cannot read properties of undefined (reading 'rename')` 报错：配置名归一化（旧名字迁移）
  跑在 `registry` / SSH 连接池创建之前，改名时的空调用被守卫捕成「操作失败」弹窗，并中断本次改名落盘与
  历史目录、转发开关、同步任务、同步配对的迁移，所以每次激活都会重复报一次。激活早期跳过这步重挂
  （两者此时都还是空的，没有旧键需要重挂），迁移一次完成、弹窗消失。
- 打开远程目录不再弹「远程配置名已更新…用新名字重新打开这个窗口吗？」：多根工作区照旧静默替换文件夹；
  单文件夹窗口 VS Code 不允许改文件夹列表，现在只往 SAFS Log 写一行提示、不打断会话（旧 authority 仍由
  别名解析到当前配置，功能不受影响，左下角指示器等用户自己重开窗口时更新）。
- 修复取消或失败的下载残留 0 字节半成品文件：`createWriteStream` 的 open 是异步的，取消时它可能还没
  落盘，删除只拿到 `ENOENT`（被 `force` 吞掉），随后 open 才把空文件建出来再没人清（实测取消 40 次残留
  39 个）。现在先销毁并等写入流真正 close（fd 释放、文件已创建）再删，删除本身对 Windows 上瞬时的
  `EBUSY`/`EPERM` 退避重试最多 5 次。测试侧：取消用例改成等读取真正开始再 abort（不再用固定 5ms——
  机器忙时它会晚于假读取），并新增「open 还在飞行中取消」的回归用例。
- 插件与原生 CLI 版本升级到 1.9.5。

## 1.9.4

- 「SAFS：可视化同步」拆成两个方向明确的入口：远程目录右键「SAFS：同步到本地」、本地文件夹右键「SAFS：同步到远程」（`resourceScheme == file && explorerResourceIsFolder`）。前者等价于"先可视化下载、再开启双向同步"，后者等价于"先可视化上传、再开启双向同步"：本地目录整棵上传到 `<所选目录>/<本地目录名>`，并以刚上传的远程状态作为基线，不会把刚上传的内容再下载回来（远程独有的内容仍会补齐到本地）。
- 同步完成的目录写入历史列表，并记住本地副本的配对关系：点击历史条目优先打开本地副本（副本已不存在时回退打开远程目录），历史条目的悬停提示与图标同时标出这是已同步的镜像。删除历史条目会一并忘掉配对。
- 同一个目录「同步一次就够」：再次执行同步时弹窗提示"该目录已在历史列表中，同步过一次了"，可选择打开本地副本、重新同步（沿用记住的两侧路径），正在同步时还可直接停止。同步未完成（取消/失败）不写历史，下一次仍是全新一次同步。
- 历史目录上的按钮文案改为「启动同步」（未同步时），同步中仍是「关闭本地同步」；该按钮对已配对的目录直接复用记住的本地副本，不再重复弹目录选择框。
- 主机节点（同一 IP 分组）保留行内 🗑「删除主机的全部账号」，确认框会列出该 IP 下的所有账号；账号行自己的 🗑 仍然只删那一条配置。三个删除入口（主机分组 / 账号·挂载 / 历史记录）都改成先弹确认框再删，删除正在同步的历史条目会一并停止该同步（本地与远程文件都不动）。
- 配置名改为 `主机名(账号)`：主机名取 ASCII 别名（如 `ws1(alice)`、`devbox(dev_alice)`），别名是中文或没配时退回 IP（如 `192.0.2.30(carol)`）。旧名字（`IP(账号)`、`账号_主机名`、更早的 `主机@账号`）在启动时自动迁移，挂载、历史目录、转发开关、同步任务与同步配对一起改名。
- 改名后已打开的窗口会把 `safs://` 工作区 URI 换成当前名字：左下角远程指示器显示的就是它。多根工作区直接替换文件夹；单文件夹窗口 VS Code 不允许改文件夹列表，会问一句是否用新名字重新打开（会重新加载窗口）。括号在 authority 里写成 `_`（`ws1(alice)` → `safs://ws1_alice/…`）。
- 修复配置改名后远程目录打不开（资源管理器里带感叹号）：VS Code 会把 authority 里的括号百分号转义（`safs://192.0.2.40%28alice%29/…`），而解析时没有还原，于是找不到挂载。现在解析 URI 时先还原 authority 的转义，并且生成的 authority 直接把括号写成 `_`（`192.0.2.10(alice)` → `safs://192.0.2.10_alice/…`），不再让 `%28` 出现在远程指示器和已保存的窗口状态里。
- URI 里的 `?mount=` 只作参考：VS Code 会把 query 里的 `=` 转义成 `%3D`（`?mount%3Dhost@user`），取不到时改为从 authority 还原名字。启动时登记两类别名——历史命名规则下的旧配置名（`账号_别名`、`账号_IP`、`别名@账号`、`IP@账号`）和当前配置名的 authority 转义形式（`192.0.2.10_alice` → `192.0.2.10(alice)`）——已保存的窗口、标签页、最近打开都不需要重开就能解析到当前配置；只登记当前不存在的名字，不会遮蔽现有配置。
- 「打开配置」按选中项定位：视图标题栏的入口取当前选中项，任意节点右键也有「打开配置」，都会打开 `~/.safs/config.json` 并把光标落到对应记录（账号密码错误提示里的入口仍直接定位到 password 字段）。
- 修复「添加账号」覆盖已有账号：主机节点（同一 IP 分组）的 ＋ 现在只追加，不再复用「单账号主机」的那条记录；同 IP 下输入已存在的账号名会弹窗确认是否更新该账号，而不是静默覆盖它的密码/私钥。
- 删除配置（主机分组或单个账号）时会一并停掉它的同步任务、清掉历史目录与本地副本配对：否则任务会留在全局状态里对着已删除的挂载按退避一直重试，而树上已经没有节点可以停它。
- 复审修正（会话中途改名的一致性）：registry / SSH 连接池 / 同步任务在改名时一起重挂到新名字下
  （重命名主机、补账号都会触发），并给新键补一份 ready 标记；持久化同步任务时不再把旧名字写回。
  改名后已打开的远程目录、正在跑的同步、镜像窗口不会再因为旧键失效。
- 复审修正（配置安全）：命名去重不再可能写出重名主机（重名会让整份 config.json 读不出来）；
  URI authority 的转义改成单射（字面下划线写成 `__`，`@` 写成 `_`），避免两个不同配置名映射到
  同一个 authority 时静默打开错主机；长得像旧十六进制形式的名字强制走十六进制兜底；
  上一版的转义形式仍作为别名保留，已保存的窗口不受影响。
- 复审修正（其它）：同 IP 同账号的本地副本冲突在「同步到本地」里也拦截；「打开配置」只在同一条
  记录里找 password 字段、找不到就落到条目行，「尚未配置账号」的错误提示也会带上主机名；
  本地侧发起的同步进度标题改成「正在同步到远程」；`mountAliasCandidates` 不再包含当前名字；
  删掉重复执行的配置归一化与多余的 `findSyncedDirectory` 包装；config.schema.json 更新命名说明。
- 清理仓库里的真实主机信息：文档、注释、测试与 schema 示例里出现过的真实账号名、主机别名、内网 IP
  与中文别名全部换成通用示例（`192.0.2.x`、`alice`/`dev_alice`、`devbox`/`gateway`/`ws1`、`测试主机`）。
  代码里不再出现任何来自个人 `config.json` 的标识。
- 复审补充：历史条目的同步状态改为同时看本窗口与持久化的任务列表（其它窗口启动的任务也能正确显示
  与停止）；同步任务键改为按平台分键并迁移旧键；删掉不可达的旧视图（`legacy` 模式）分支与零引用的
  `commandSucceeds`。
- 插件与原生 CLI 版本升级到 1.9.4。

## 1.9.3

- 远程终端在内置 `ssh2` 握手、网络或通道失败时静默回退系统 SSH。
- 远程目录新增三级兼容链路：`ssh2 + SFTP`、`ssh2 + SCP/exec`、系统 OpenSSH + SCP/exec。
- 扩大首次 SSH 握手瞬时故障重试范围，SFTP 子系统无响应时自动回退 SCP/exec。
- 修复无权限远程目录缺少提示与退出的问题。
- 可视化大文件传输改用严格背压，并增大有界 SFTP 读取窗口以兼顾内存和吞吐。
- 插件与原生 CLI 版本升级到 1.9.3。

## 1.9.2

- 远程挂载浏览范围改为 `/`，默认打开登录家目录；支持打开／切换家目录外路径，并兼容旧窗口路径恢复。
...
- 创建操作拒绝越界路径、符号链接逃逸、覆盖已有目标和创建工作区根目录。
- Agent 活动记录新增创建文件/目录操作，并仅保存内容字节数。
- 插件与原生 CLI 版本升级到 1.9.1。

## 1.9.0

- 新增 SAFS 远程目录新视图，按主机名、账号和历史目录组织远程工作区。
- 同 IP 主机合并显示，账号配置名自动生成为“账号_主机名”，并修复主机节点状态显示。
- 中文主机别名仅用于显示，配置名回退为“账号_IP”，避免远程 URI 兼容问题。
- 插件与原生 CLI 版本升级到 1.9.0。

## 1.8.9

- 终端模式与工作区模式按窗口分别显示；在新窗口打开远程目录时保留原窗口的终端模式。
- 修复 Agent 活动面板在没有生效模式时仍显示空白模式详情的问题，并移除无用状态变量。
- 终端模式下的 Agent 活动记录覆盖远程命令，并修复终端工作区绑定。
- 插件与原生 CLI 版本升级到 1.8.9。
...
- 原生 `safs` CLI 改用更直接的子命令语法，并新增 Playwright CLI 风格的
  `safs install --skills`，可离线安装项目级或用户级 SAFS Agent Skill。
- 删除 Agent 混合接口，仅保留互斥的 MCP 与 CLI：MCP 模式清理全局 CLI 和 SAFS
  Skill，CLI 模式安装 CLI/Skill 并复制 Agent 侧 MCP 注销提示词。
- Agent 转发安装改为所有连接共享一次：仅在全部连接均关闭后首次开启连接时提示，继续
  开启其他连接不再重复安装；不同连接的开关操作串行处理以避免并发重复提示。
- 终端专用模式会跟踪终端当前目录并自动刷新工作区；再次点击终端模式即可刷新或改选
  当前聚焦的 SAFS 终端，无需先切回工作区模式。
- 插件与原生 CLI 版本升级到 1.8.7。

## 1.8.6

- Agent 活动面板将工作区模式与终端模式改为并排的二选一控件；终端模式下 MCP 与 CLI
  拒绝其他操作时，会明确提示唯一可用的 `run_remote_command` 工具或 `safs exec` 命令。
- 修复首次 SSH/SFTP 建连阶段偶发 `ECONNRESET` 时直接打开失败的问题，连接级瞬时故障
  会短暂退避并自动使用全新连接重试一次。
- 修复无 SFTP 子系统时 SCP/exec 回退受远端语言环境影响而误判目录类型的问题；默认
  远程路径 `.`、`./` 和 `~` 会显式解析为 SSH 登录用户家目录，不再受 exec 初始目录影响。
- 插件与原生 CLI 版本升级到 1.8.6。

## 1.8.5

- Agent 活动面板新增会话级终端专用模式：读取当前 SAFS 终端的实际目录并作为工作区，
  MCP 仅保留参数收缩后的 `run_remote_command`，CLI 绑定无需重建且只放行 `safs exec`。
- 终端专用模式继承终端内通过 `sudo`/`su` 切换后的用户权限和已导出环境；关闭终端或退出
  专用模式后自动恢复原有 MCP/CLI 工具，文件、搜索、传输及批处理在专用模式下均被拒绝。
- 修复瞬时 SSH 握手丢失、连接重置或超时后终端过早停止自动恢复的问题，改为有限退避重试；
  普通及认证错误仍快速停止。
- 打开或切换远程目录命令现在也可从本地或空窗口先选择连接再执行。
- 插件与原生 CLI 版本升级到 1.8.5。

## 1.8.4

- 代理提示准确描述代理环境变量与本机地址绕过配置，不再将规则代理误称为全局代理或断言转发一定受影响。
- 代理提示新增“测试本机连接”按钮与 `safs.testLocalProxy` 命令，通过 curl 实测三个回环地址的连通性及显式代理使用情况。
- 修复测试在 Windows 下的换行符、路径和权限位断言；缺少符号链接权限或依赖 Unix 环境的测试明确标记跳过。
- 插件与原生 CLI 版本升级到 1.8.4。

## 1.8.3

- Agent 接口默认值改为 `mcp`，纯 MCP 模式不再安装全局 CLI；切换到混合或 CLI 模式时
  才安装，并新增 `SAFS: 安装或更新全局 CLI` 命令供用户在任意模式下主动安装或修复。
- 删除无实际路由意义的 `safs.agentPlatform` 设置、安装时的平台选择，以及 URL、绑定、
  活动记录和审计日志中的 Agent 平台字段；Agent 身份只保留名称。
- 首次启动插件时检查标准全局代理环境变量；若 `NO_PROXY` 未覆盖本机回环地址，提示设置
  `NO_PROXY=localhost,127.0.0.1,::1`。MCP 安装提示不再推荐规则模式或切换 CLI。
- 首次从连接父节点打开配置的远程家目录时，按当前平台提示“打开远程目录 / 终端”的
  两个快捷键；历史目录和后续打开不再重复提示。
- 插件与原生 CLI 版本升级到 1.8.3；发布流水线继续重建并校验六个平台二进制。

## 1.8.2

- Agent 活动时间线已按最新操作优先显示，因此删除重复且易错过的“关键操作”弹幕区域
  及无实际价值的“暂停弹幕”按钮；页面顶部状态卡改用更精致的 macOS 圆角窗口风格，
  侧边栏图标保持不变。
- 删除原生 CLI 中已废弃的 `mcp-bridge` stdio 兼容入口；MCP 统一由 Agent 直接连接
  loopback Streamable HTTP 路由。
- Agent 安装提示只在目录树父连接节点首次从关闭切换为开启时，或用户主动执行命令面板
  中的安装命令时显示一次；窗口启动和接口模式变化仅静默准备路由与 CLI，避免多个窗口
  重复弹出 Agent 名称输入框。
- 同一逻辑工作区重新发布且目标唯一时自动续接已有绑定，避免窗口后端切换后重复要求
  用户确认；若绑定确实失效，重新获取时也会优先恢复该 Agent 上次选择的同一目标。
- 混合模式的绑定结果通过唯一的 `cliInstructions` 字段携带 CLI 文件操作约束，移除重复
  状态及与 CLI 调用冲突的 Shell 禁用字段；工作区边界错误改为不可重试的结构化错误，
  明确禁止回退到 `safs exec`，原生 CLI 也会显示命令执行前的拒绝原因。
- 重新梳理混合与纯 MCP 的工具契约：拆分 CLI/MCP 会话说明，精简纯 MCP 绑定和切换
  结果，明确本地 cwd、候选确认、分页、读取选择器、传输目录、命令退出码及输出续读；
  活动文件、搜索和传输结果不再返回重复的内部字段，公开参数校验错误统一使用英文。
- 统一 MCP、混合和 CLI 模式的写入边界：`workspaceRoot` 外的显式远程 Shell 工作目录
  与常见文件写入目标会被后端拒绝；远程文件操作必须使用结构化命令，`safs exec`
  仅用于构建、测试等任务命令。
- MCP 与混合模式共用同一份英文安装提示词，不再重复声明 MCP 会话已提供的规则；
  纯 CLI 模式使用英文提示词将唯一的 SAFS 标记块保存到用户级全局持久指令，重复安装
  时替换而非追加，卸载时按标记只清理对应规则。
- Bash Shell Integration 仅在真实交互会话中加载登录配置，避免非交互环境注入的
  `PROMPT_COMMAND` 破坏状态捕获与命令生命周期报告。
- 插件与原生 CLI 版本升级到 1.8.2；发布流水线继续重建并校验六个平台二进制。
...
- Agent 接口新增并默认使用 `hybrid` 混合模式：MCP 只注册工作区绑定和切换两个工具，
  ...
  同时保留精确的工作区选择。
- 升级首次激活时将用户设置中旧版显式保存的 `mcp` 一次性迁移为 `hybrid`；显式
  `cli` 保持不变，迁移完成后仍可手动切换任意模式。
...
- CLI 安装不再绑定 Agent 名称；`safs bind --agent NAME`（以及创建新 binding 的
  ...
  和转发状态栏不再显示 `safs-cli`。
- MCP/CLI 安装提示词缩减为必要的安装地址或 `safs --help` 入口，不再要求 Agent
  记忆工作流规则；远程目录与 Agent 活动视图使用不同图标，分离后仍可辨认。
- 默认 MCP 恢复为 loopback Streamable HTTP，不再安装或使用原生 stdio 桥；若全局
  代理导致 HTTP 502、代理连接错误或超时，安装提示会要求使用代理规则模式并让
  `127.0.0.1`、`localhost`、`::1` 直连，或将 Agent 接口切换为 CLI。
- 新增 SAFS **Agent 活动**侧边栏：以状态动画、关键操作弹幕和可筛选时间线展示
  当前远程窗口实际执行的 MCP/CLI 文件操作、命令和传输；连续只读操作自动折叠。
- 每个工作区持久保留最近 200 条脱敏活动记录；不保存文件正文、Diff 或命令输出，
  窗口重载时仍未完成的操作标记为已中断，清空活动不影响原有审计日志。
- CLI 安装转发时复制全局 `safs` 使用提示词；MCP 与 CLI 来源标签会传递到实际执行
  窗口，活动生命周期按开始、成功、失败完整配对，批处理逐项记录。
- 插件与原生 CLI 版本升级到 1.8.1，并由发布流水线重新构建六个平台二进制。
...
- CLI 请求失败时保留经过 Token 脱敏的底层错误，不再把 HTTP 状态、连接失败和
  真正超时全部折叠为同一条错误信息。
- 精简 Agent、同步、传输、终端和错误处理的非模态通知，完整路径与诊断写入
  SAFS 日志，避免窄窗口把关键操作提示截断。
- 首次使用原生 SAFS CLI/MCP 桥时校验磁盘二进制的真实版本；与插件版本不一致
  或旧版无法报告版本时自动更新，并在更新后再次校验。
- 六个平台的原生 `safs` 二进制全部随 VSIX 分发，版本修复直接从插件包内原子
  替换，不再依赖运行时访问 GitHub 下载。
...
- `current-file` 同时支持单次请求的 CLI 批处理，并在没有打开远程文件时正确返回 `null`。
- CLI 新增按子命令展开的精确帮助、正常成功的工作区列表和本地请求超时；切换工作区不再误注销同平台其他 Agent 会话的绑定，自动刷新 bin 后会记录平台和安装路径。

## 1.7.5

- CLI 模式切换后立即安装或修复用户级全局 `safs` 命令，显示安装路径并提示重启 VS Code。
- 新增紧凑输出、默认精简错误、`--verbose` 完整诊断，以及单次本地 HTTP 请求的顺序批处理。
- 修复 Windows 到 WSL 的 CLI 权限设置，并补充 zsh 登录环境的 PATH 配置。
- 文档明确 CLI 的 Token 特性，以及自动安装和手动安装 MCP 的不同清理方式。

## 1.7.4

- Agent 转发继续默认使用稳定的 MCP 接口；原生 CLI 保留为实验性可选入口，只有安装包
  包含 Agent 平台对应的可执行文件时才启用。
- MCP 文件读取、搜索和命令输出增加默认预算、分页及续取能力，并提供精简的 `core`
  工具配置，减少远程操作进入 Agent 上下文的内容量。
- 新增复用现有认证、路由和安全策略的原生 SAFS CLI，以及 Windows、macOS、Linux
  x64/ARM64 构建流水线；本地打包允许只携带当前已构建的平台程序，发布流水线仍校验
  六个平台产物完整。
- CLI 模式把原生程序安装为用户级全局 `safs` 命令并自动维护 PATH；CLI 从程序旁读取
  私有连接配置，不再向 AGENTS.md、CLAUDE.md 或剪贴板写入 Agent 使用指引。

## 1.7.3

- 上传、下载及首次可视化同步结束后显示独立完成通知，包含文件数量、大小和目标路径，
  同时记录到日志，避免小文件进度一闪即逝；后台增量同步不重复弹窗。

- 可视化同步和历史目录同步共享首次边遍历边下载流程（SFTP 4 路、SCP 单路），
  全部成功后才发布基线；增量扫描失败保留旧指纹，不按不完整结果删除文件。
  同步上传采用临时文件替换，下载保留本地编辑检查，并修复同路径下载排队竞态。

- 可视化上传改为边遍历边传输，SFTP 最多 4 路并发、SCP 单路，按需创建并复用目录；
  文件先写唯一临时路径再替换目标，失败或取消清理临时文件，保留未替换的原文件。
  SFTP 优先使用原子重命名，不支持时安全回退，禁止预先删除旧目标。

- 可视化目录下载改为边发现边下载，无需等待全量递归扫描；SFTP 使用有背压的 4 路
  并发，SCP/exec 回退为保证旧网关稳定性自动串行。连接、发现和下载阶段均立即显示
  可取消的   进度通知，并纳入 Agent 下载超时。

- 修复扩展运行时状态中 `safs.mounts.focus` 菜单命令未声明的清单校验错误。
...
- Fix `Packet length … exceeds max length` on NSG gateways that inject a MOTD
  banner into every SSH channel (e.g. 192.0.2.20: `
...
- SCP fallback filesystem: when the server has no SFTP subsystem (e.g. NSG
  gateways running old OpenSSH without sftp-server, like 192.0.2.10), the
