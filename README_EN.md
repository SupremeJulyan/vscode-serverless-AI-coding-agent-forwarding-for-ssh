...
5. Click **+** beside a host name to enter the account and password. Passwords use the configuration encryption master password when needed; private-key authentication is also supported. The configuration name is generated as `host(account)`, where **host is the alias** (`ws1(alice)`, `gateway(ops_carol)`) and falls back to the IP for a Chinese or missing alias (`192.0.2.30(carol)`). The **+** on a host node always appends an account; it never overwrites an existing one for the same IP.
6. On the first connection, verify and accept the server's host-key fingerprint.

The **🗑** on a host node (the entry that merges one IP) deletes every account under it along with their mounts; the **🗑** on an account node deletes just that account. Both ask for confirmation first, and deleting a configuration also stops its sync tasks and drops its history entries and local-copy pairs. **Open Config** on any node opens `~/.safs/config.json` with the cursor placed on that entry (the title-bar **Open Config** uses the current selection).
...
Configuration names generated by older versions (`IP(account)`, `account_host name`, and the even older `host@account`) are migrated to `host(account)` on startup together with their mounts, history entries, forwarding flags, sync tasks, and sync pairs. Parentheses are written as `_` inside the URI (`ws1(alice)` → `safs://ws1_alice/…`), with the full name also stored in the URI's `?mount=` parameter so it can be recovered from the authority even when that query is escaped away. Open windows are renamed too (the remote indicator in the lower left shows that authority): a multi-root workspace swaps the folder in place, while a single-folder window is asked whether to reopen with the new name. Windows and tabs that still carry an old name keep working without being reopened.
