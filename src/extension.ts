...
let mcpBoundMountName: string | undefined;
...
let agentActivityView: AgentActivityViewProvider;
...
function updateSafsStatusBar(
  agentFocus = false, agentName?: string, clearSource = false
): void {
  ...
  if (agentName) focusedAgentSource = { name: agentName };
  const source = focusedAgentSource?.name ?? 'Agent';
...
function backendMcpToolProfile(): 'full' | 'terminal' {
  return activeAgentCommandTerminal() ? 'terminal' : 'full';
}
...
function scheduleFirstProxyEnvironmentCheck(context: vscode.ExtensionContext): void {
  let checking = false;
  const check = async (): Promise<void> => {
    if (checking || !vscode.window.state.focused) return;
    if (context.globalState.get<boolean>(proxyEnvironmentCheckKey, false)) {
      listener.dispose();
      return;
    }
    checking = true;
    try {
      // Mark the first focused activation before opening UI so another window
      // cannot display the same first-install warning at the same time.
      await context.globalState.update(proxyEnvironmentCheckKey, true);
      listener.dispose();
      const warning = proxyEnvironmentWarning(process.env);
      if (!warning) return;
      bridgeOutput?.warn(
        `[代理检测] 检测到 ${warning.proxyVariables.join(', ')}；NO_PROXY 缺少 ${
...
function activeAgentCommandTerminal(): vscode.Terminal | undefined {
  ...
  return !hasRemoteWorkspaceContext() && terminal && agentCommandTerminalCwd
    && managedRemoteTerminals.has(terminal) ? terminal : undefined;
}
...
function agentCommandTerminalState(): {
  ...
  return {
    enabled: true,
    mode: 'terminal',
    label: `${terminal.name} · ${agentCommandTerminalCwd}`
  };
}
...
async function setAgentCommandTerminalMode(enabled: boolean): Promise<void> {
  ...
  await enableAgentCommandTerminal(terminal, info, refreshingSelectedTerminal);
}

async function enableAgentCommandTerminal(
  terminal: vscode.Terminal,
  info: NonNullable<ReturnType<typeof managedRemoteTerminals.get>>,
  forceProbe = false
): Promise<void> {
  ...
  try {
    ...
    terminalCwd = await probeAgentCommandTerminalCwd(terminal, info, forceProbe);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
  ...
  await refreshAgentCommandTerminalState();
  await publishAgentWorkspace(vscodeContext);
  const action = forceProbe ? '已刷新终端专用模式' : '已启用终端专用模式';
...
async function prepareOpenedTerminalForAgent(
  terminal: vscode.Terminal,
  info: NonNullable<ReturnType<typeof managedRemoteTerminals.get>>
): Promise<void> {
  ...
  const forwarding = vscodeContext.globalState
    .get<string[]>(aiForwardMountsKey, []).includes(info.mount.name);
...
async function openTerminal(
  ...
  if (!forceNew) {
    ...
    if (existingTerminal) {
      ...
      const existingInfo = managedRemoteTerminals.get(existingTerminal);
      if (existingInfo) {
        await prepareOpenedTerminalForAgent(existingTerminal, existingInfo);
      }
  ...
  try {
    ...
    terminal.show();
    await prepareOpenedTerminalForAgent(terminal, managedRemoteTerminals.get(terminal)!);
...
async function ensureAgentMcpServer(context: vscode.ExtensionContext): Promise<AgentMcpServer> {
  const location = hasRemoteWorkspaceContext() ? currentRemoteLocation() : undefined;
  const selectedTerminal = activeAgentCommandTerminal();
  const terminalInfo = selectedTerminal ? managedRemoteTerminals.get(selectedTerminal) : undefined;
  const boundMountName = location?.mountName ?? terminalInfo?.mount.name;
  if (!boundMountName) throw new Error('当前窗口不是 Serverless Remote 工作区');
  if (mcp && mcpBoundMountName !== boundMountName) {
    await agentWorkspacePublisher.remove();
    await mcp.stop();
    mcp = undefined;
    mcpBoundMountName = undefined;
  }
  if (!mcp) {
      ...
      token,
      {
        ...
        currentWorkspace: async () => {
          ...
          const terminalInfo = agentCommandTerminal
            ...
            : undefined;
          const mountName = location?.mountName ?? terminalInfo?.mount.name;
          if (!mountName) return null;
          const enabled = new Set(context.globalState.get<string[]>(aiForwardMountsKey, []));
          if (!enabled.has(mountName)) return null;
          const config = await readConfig();
          const mount = config.mounts.find((candidate) => candidate.name === mountName);
          if (!mount) return null;
          const folder = await ensureFolder(mount);
          const editorWorkspacePath = currentWorkspacePath(folder);
          const workspacePath = activeAgentCommandTerminal() && agentCommandTerminalCwd
        ...
        activity: {
          start: (entry) => {
            ...
            return agentActivityStore.start({
              ...
              workspaceRoot: activeAgentCommandTerminal() && agentCommandTerminalCwd
                ? agentCommandTerminalCwd
                : currentWorkspacePath(folder)
            });
          },
          succeed: (id, result) => agentActivityStore.succeed(id, result),
          fail: (id, error) => agentActivityStore.fail(id, error)
        },
    ...
    mcpBoundMountName = boundMountName;
...
async function publishAgentWorkspace(context: vscode.ExtensionContext): Promise<void> {
  ...
  const mountName = location?.mountName ?? terminalInfo?.mount.name;
  const enabled = new Set(context.globalState.get<string[]>(aiForwardMountsKey, []));
  if (mountName && enabled.has(mountName) && mcpBoundMountName !== mountName) {
    await ensureAgentMcpServer(context);
  }
  if (!mountName || !enabled.has(mountName) || !mcp?.running || mcp.portUnavailable) {
    updateSafsStatusBar(false, undefined, true);
    if (mountName && !enabled.has(mountName)) await mcp?.stop();
    await agentWorkspacePublisher.remove();
    const reason = !mountName ? '非远程工作区'
      : !enabled.has(mountName) ? `挂载 ${mountName} 未启用 Agent 转发`
  ...
  const editorWorkspacePath = currentWorkspacePath(folder);
  const terminalCommandOnly = activeAgentCommandTerminal() !== undefined;
...
async function probeInstalledCliVersion(executable: string): Promise<string | undefined> {
  try {
    const result = await executeCaptured(
...
async function installGlobalCli(
  ...
  if (nativePlatform.startsWith('win32-')) {
    ...
    await ensureUnixCliPath(agentHome);
  }
...
async function askAgentName(title: string): Promise<string | undefined> {
...
async function installAgentForwardingIntegration(
  ...
  const agentName = await askAgentName('SAFS：为我的Agent安装转发功能');
  if (!agentName) return;
  const router = await ensureAgentHttpRouter(context);
  const url = agentTaggedMcpUrl(router.url, agentName);
...
async function updateAiForwardEnabled(
  ...
  await vscode.window.withProgress({
    ...
    const terminalInfo = agentCommandTerminal
      ? managedRemoteTerminals.get(agentCommandTerminal)
      : undefined;
    const activeMountName = current?.mountName ?? terminalInfo?.mount.name;
    if (!enabledValue && activeMountName === mount.name) {
...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  agentActivityView = new AgentActivityViewProvider(agentActivityStore, {
    ...
    refreshTerminalTarget: refreshAgentCommandTerminal,
    switchWorkspace: async () => {
      await vscode.commands.executeCommand<void>(`${commandPrefix}.switchRemoteDirectory`);
    }
  });
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeWorkspaceFolders(() => {
      ...
      if (hasRemoteWorkspaceContext() && agentCommandTerminal) {
        void setAgentCommandTerminalMode(false).catch((error) =>
          logAsyncFailure('切换工作区模式失败', error)
        );
        return;
      }
      void refreshAgentCommandTerminalState();
